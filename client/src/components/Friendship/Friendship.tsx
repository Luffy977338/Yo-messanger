...
const Friendship = () => {
  ...
  const subscribe = useSubscribe(id as string);

  const isFriend = () => {
    if (data && data.friends && !isLoading) {
      return data.friends.some((friend: IUser) => friend._id === user.user._id);
    }
    return false;
  };
  ...
  const isSubscriber = () => {
    if (data && data.subscriptions && !isLoading) {
      return data.subscriptions.some((sub: IUser) => sub._id === user.user._id);
    }
    return false;
  };
